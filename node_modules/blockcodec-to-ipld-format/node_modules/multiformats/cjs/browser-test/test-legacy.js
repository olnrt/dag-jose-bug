...
const hashes = {
  [sha2Browser.sha256.name]: sha2Browser.sha256,
  [sha2Browser.sha512.name]: sha2Browser.sha512
};
describe('multicodec', () => {
  let raw$1;
  let json$1;
  ...
  let link;
  before(async () => {
    raw$1 = legacy.legacy(raw, { hashes });
    json$1 = legacy.legacy(json, { hashes });
  ...
  test('cid', async () => {
    ...
    same(cid.version, 1);
    same(cid.codec, 'raw');
    const {bytes} = await sha2Browser.sha256.digest(buffer.Buffer.from('test'));
